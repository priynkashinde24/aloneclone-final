...
async function generateAttributionSnapshots(
  ...
  try {
    ...
    for (const model of models) {
      ...
      const channelOrderMap = new Map<string, { count: number; revenue: number }>();
      ...
      for (const order of orders) {
        ...
        let channel: string | undefined;
        if (model === 'first_touch') {
          channel = order.marketingAttribution.firstTouch?.channel;
        } else if (model === 'last_touch') {
          channel = order.marketingAttribution.lastTouch?.channel;
        } else {
          // Multi-touch: use channel with highest credit
          if (order.marketingAttribution.channelCredits && order.marketingAttribution.channelCredits.length > 0) {
            const topCredit = order.marketingAttribution.channelCredits.reduce((max, c) =>
              c.credit > max.credit ? c : max
            );
            channel = topCredit.channel;
          } else {
            channel = order.marketingAttribution.lastTouch?.channel;
          }
        }
        ...
        if (channel) {
          ...
          existing.count += 1;
          existing.revenue += order.grandTotal || order.totalAmountWithTax || 0;
