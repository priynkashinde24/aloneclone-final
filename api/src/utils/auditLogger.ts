...
export interface AuditLogParams {
  ...
  actorRole?: 'admin' | 'supplier' | 'reseller' | 'system'; // Actor role (optional, extracted from req.user if not provided)
...
export const logAudit = async (params: AuditLogParams): Promise<void> => {
  try {
    ...
    let actorRole: 'admin' | 'supplier' | 'reseller' | 'system' = providedActorRole || 'system';
    ...
    const sanitizePayload = (payload: Record<string, any> | null | undefined) => {
      ...
      return maskAndTruncate(payload, 10_000); // cap at ~10KB
    };
