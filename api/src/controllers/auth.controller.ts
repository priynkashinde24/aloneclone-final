...
export const register = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Admin users are active and approved by default, others need approval
    const isAdmin = role === 'admin';
    const isDevelopment = process.env.NODE_ENV !== 'production';
    
    // In development, auto-approve all users for easier testing
    // In production, only admins are auto-approved
    const shouldAutoApprove = isAdmin || isDevelopment;
    ...
    const user = new User({
      ...
      approvedBy: null, // Will be set when admin approves (or null for auto-approval)
...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (connectionState !== 1) {
      console.log('[LOGIN] MongoDB not connected, attempting connection...');
      ...
      try {
        ...
        console.log('[LOGIN] MongoDB connected successfully');
      } catch (dbError: any) {
    ...
    if (!user.isActive) {
      // Audit log: Login failed (inactive)
      const { logAudit } = await import('../utils/auditLogger');
      await logAudit({
        req,
      ...
      sendError(res, 'Account disabled. Contact admin.', 403);
      return;
    }
    ...
    // In development mode: auto-approve pending users
    if (isDevelopment && !isApproved && !isAdmin) {
      console.log('[LOGIN] Development mode: Auto-approving pending user:', user.email);
    ...
    // In production: block login if not approved
    if (!isDevelopment && !isAdmin && !isApproved) {
    ...
    // Verify user has access to the store (if store was resolved)
    if (store && resolvedStoreId) {
      ...
      if (user.role !== 'admin') {
        ...
        const hasAccess = user.accessibleStores.some(
          (storeId) => storeId.toString() === resolvedStoreId
        ) || user.defaultStoreId?.toString() === resolvedStoreId;
        
        // Also check if user is the store owner
        const isOwner = store.ownerId === user._id.toString();
      ...
      // Non-admin users must have a store (unless they're admin)
      // For now, allow login without store (store can be set later)
      console.log('[LOGIN] No store resolved, but allowing login (store can be set later)');
    }
    ...
    console.log('[LOGIN] Tokens issued successfully:', { userId: user._id.toString(), role: user.role });
...
export const sendOTP = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Don't reveal if user exists (security best practice)
    sendSuccess(res, null, 'If an account exists with this phone number, an OTP has been sent.');
  } catch (error) {
    next(error);
  }
};
