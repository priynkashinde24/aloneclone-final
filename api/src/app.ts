...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    res.status(503).json({
      success: false,
      message: 'Database connection failed. Please verify MONGODB_URI is set in Vercel environment variables.',
      hint: !process.env.MONGODB_URI 
        ...
        : 'Check MongoDB Atlas Network Access and connection string format.',
...
// Storefront routes - require store resolution
app.use('/api/storefront', resolveStore, storefrontRoutes);
